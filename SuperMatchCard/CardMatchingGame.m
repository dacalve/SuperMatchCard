...
//                }
                ...
                //Make the card determine if it is more than 2 card match.
                //So multipass match is on the card level.
                
                matchScore += [card match:workingCards];
                
                if (matchScore) {
                    self.lastScore = matchScore * MATCH_BONUS;
                    self.score += self.lastScore;
                    card.matched = YES;
                    [self matchCurrentCards:workingCards];
                    
                } else {
                    self.lastScore = -MISMATCH_PENALTY;
                    self.score += self.lastScore;
                    card.matched = NO;
                    [self unchooseCurrentCards:workingCards];
                }
...
-(void)matchCurrentCards:(NSMutableArray *)currentCards
{
...
-(void)unchooseCurrentCards:(NSMutableArray *)currentCards
{
    for (Card *otherCard in currentCards) {
        otherCard.chosen = NO;
    }
